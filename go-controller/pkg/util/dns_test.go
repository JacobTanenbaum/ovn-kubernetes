...
func TestAddDNS(t *testing.T) {
	...
	tests := []dnsTest{
		...
		},
		{
			...
			domainName:        "example.com",
			dnsResolverOutput: "example.com. 600 IN AAAA 2001:0db8:85a3:0000:0000:8a2e:0370:7334",
			ips:               []net.IP{ipv6},
			ttl:               600,
			expectFailure:     false,
		},
		{
			testCase:          "Test valid domain name with resolver returning both AAAA and A record",
			domainName:        "example.com",
			dnsResolverOutput: "example.com. 600 IN AAAA 2001:0db8:85a3:0000:0000:8a2e:0370:7334\nexample.com. 600 IN A 10.11.12.13",
			ips:               []net.IP{ipv6, ip},
			ttl:               600,
			expectFailure:     false,
		},
		{
			testCase:          "Test valid domain name with resolver returning both AAAA and A record taking the minTTL",
			domainName:        "example.com",
			dnsResolverOutput: "example.com. 600 IN AAAA 2001:0db8:85a3:0000:0000:8a2e:0370:7334\nexample.com. 200 IN A 10.11.12.13",
			ips:               []net.IP{ipv6, ip},
			ttl:               200,
			expectFailure:     false,
		},
