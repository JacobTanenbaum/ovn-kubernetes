...
func (e *EgressDNS) Add(namespace, dnsName string) (AddressSet, error) {
	...
	if _, exists := e.dnsEntries[dnsName]; !exists {
		...
		if e.addressSetFactory == nil {
			panic("addressSetFactory is null, that cannot happen")
		}
...
func (e *EgressDNS) Run(stopChan <-chan struct{}, defaultInterval time.Duration) {
	var dnsName string
	//intially the next DNS Query happens at the default interval
	durationTillNextQuery := defaultInterval
	var nextQueryTime time.Duration
	go func() {
		for {
			...
			nextQueryTime, dnsName = e.GetNextQueryTime(defaultInterval)
			if nextQueryTime > defaultInterval {
				durationTillNextQuery = defaultInterval
			} else {
				durationTillNextQuery = nextQueryTime
			}
		}
	}()
...
func (e *EgressDNS) GetNextQueryTime(defaultInterval time.Duration) (time.Duration, string) {
	tm, dnsName, timeSet := e.dns.GetNextQueryTime()
	if !timeSet {
		//No DNS entries; wait a long time or until one is added
		return defaultInterval, ""
	}
	now := time.Now()
	if tm.After(now) {
		// Wait until the soonest entry refresh interval
		return tm.Sub(now), ""
	}

	// Refresh this name now
	return 0, dnsName
